...
export default isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    // Parse url
    const fullPath = buildFullPath(own('baseURL'), own('url'), own('allowAbsoluteUrls'));
    ...
    if (configSocketPath) {
      ...
      setProxy(options, own('proxy'), protocol + '//' + parsed.hostname + (parsed.port ? ':' + parsed.port : '') + options.path);
    }
      ...
      if (responseType === 'stream') {
        ...
        responseStream.on('data', function handleStreamData(chunk) {
          ...
          if (maxContentLength > -1 && totalResponseBytes > maxContentLength) {
            ...
            abort(new AxiosError('maxContentLength size of ' + maxContentLength + ' exceeded',
    ...
    if (utils.isStream(data)) {
      ...
      let uploadStream = data;
        if (maxBodyLength > -1 && maxRedirects === 0) {
